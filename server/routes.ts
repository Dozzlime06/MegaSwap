...
export async function registerRoutes(
  ...
  app.get("/api/swap/quote", async (req, res) => {
    try {
      ...
      if (direction === 'usdm-to-eth') {
        // USDM -> ETH
        const ethOut = (amountNum / ethPrice) * (1 - fee);
        res.json({
          inputAmount: amount,
          inputToken: 'USDM',
          outputAmount: ethOut.toFixed(6),
          outputToken: 'ETH',
          rate: ethPrice,
          fee: '0.3%',
          feeAmount: ((amountNum / ethPrice) * fee).toFixed(6),
        });
      } else {
        // ETH -> USDM
        const usdmOut = amountNum * ethPrice * (1 - fee);
        res.json({
          inputAmount: amount,
          inputToken: 'ETH',
          outputAmount: usdmOut.toFixed(2),
          outputToken: 'USDM',
          rate: ethPrice,
          fee: '0.3%',
          feeAmount: (amountNum * ethPrice * fee).toFixed(2),
        });
      }
    } catch (error: any) {
  ...
  app.post("/api/swap", async (req, res) => {
    try {
      const { from, to, amount, address, txHash } = req.body;
      ...
      if (from === 'ETH' && to === 'USDM') {
        outputAmount = (parseFloat(amount) * ethPrice * (1 - fee)).toFixed(2);
      } else {
        outputAmount = (parseFloat(amount) / ethPrice * (1 - fee)).toFixed(6);
      }
      
      // Log swap for records
      console.log(`[Swap] ${address}: ${amount} ${from} -> ${outputAmount} ${to}`);
  ...
  app.get("/api/wormhole/status", async (req, res) => {
    try {
      const WORMHOLE_CORE = '0xaBf89de706B583424328B54dD05a8fC986750Da8';
      const MEGAETH_RPC = 'https://mainnet.megaeth.com/rpc';
      
      const [codeRes, chainIdRes, gsIndexRes] = await Promise.all([
        fetch(MEGAETH_RPC, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ jsonrpc: '2.0', method: 'eth_getCode', params: [WORMHOLE_CORE, 'latest'], id: 1 }),
        }),
        fetch(MEGAETH_RPC, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ jsonrpc: '2.0', method: 'eth_call', params: [{ to: WORMHOLE_CORE, data: '0x9a8a0592' }, 'latest'], id: 2 }),
        }),
        fetch(MEGAETH_RPC, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ jsonrpc: '2.0', method: 'eth_call', params: [{ to: WORMHOLE_CORE, data: '0x1cfe7951' }, 'latest'], id: 3 }),
        }),
      ]);
      
      const [codeData, chainIdData, gsIndexData] = await Promise.all([
        codeRes.json(),
        chainIdRes.json(),
        gsIndexRes.json(),
      ]);
      ...
      const wormholeChainId = chainIdData.result ? parseInt(chainIdData.result, 16) : null;
      const guardianSetIndex = gsIndexData.result ? parseInt(gsIndexData.result, 16) : null;
      
      const TOKEN_BRIDGE = '0xF97B81E513f53c7a6B57Bd0b103a6c295b3096C5';
      const bridgeCodeRes = await fetch(MEGAETH_RPC, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ jsonrpc: '2.0', method: 'eth_getCode', params: [TOKEN_BRIDGE, 'latest'], id: 4 }),
      });
      const bridgeCodeData = await bridgeCodeRes.json();
      const tokenBridgeDeployed = bridgeCodeData.result && bridgeCodeData.result !== '0x';
      
      res.json({
        deployed,
        contract: WORMHOLE_CORE,
        wormholeChainId,
        guardianSetIndex,
        tokenBridge: TOKEN_BRIDGE,
        tokenBridgeDeployed,
        mayanSwiftDeployed: false,
        timestamp: Date.now(),
      });
    } catch (error: any) {
      res.status(500).json({ error: error.message || 'Failed to check Wormhole status' });
    }
  });
