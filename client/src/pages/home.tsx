...
import { BRIDGE_OUT_ADDRESS, MAX_DEPOSIT, SUPPORTED_CHAINS, MEGAETH_CONFIG, ChainConfig, WORMHOLE_TOKEN_BRIDGE } from "@/lib/contract";
  ...
  const handleBridge = async () => {
    ...
    try {
      ...
      if (isBridgeIn) {
        ...
        const recipientBytes32 = '0x000000000000000000000000' + activeWallet.address.slice(2).toLowerCase();
        const nonce = Math.floor(Math.random() * 1000000);
        const data = iface.encodeFunctionData('wrapAndTransferETH', [
          2,
          recipientBytes32,
          BigInt(0),
          nonce
        ...
        const hash = await provider.request({ 
          ...
          params: [{ 
            ...
            to: WORMHOLE_TOKEN_BRIDGE, 
            value: "0x" + BigInt(Math.floor(amountNum * 1e18)).toString(16),
            data: data
          }] 
        });
        setTxHash(hash as string); setBridgeSuccess(true); setAmount(""); 
        toast({ title: "Bridge Out via Wormhole!", description: "~15 min to receive on Ethereum" });
